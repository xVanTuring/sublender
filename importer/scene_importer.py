        ...
        if sbs_pkg is not None:
            ...
            for graph_info in sbs_pkg["graphs"]:
                ...
                if self.from_library:
                    ...
                    importing_graph.library_uid = "{}_{}".format(
                        label, sbs_pkg["asmuid"]
                    )
                    active_material = context.scene.sublender_library.active_material
                    if (
                        len(
                            utils.globalvar.library_material_preset_map.get(
                                active_material
                            )
                        )
                        > 0
                        ...
                        if (
                            ...
                            importing_graph.material_name = new_material_name(
                                importing_graph.preset_name
                            )
...
class SublenderOTImportGraph(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for importing_graph in importing_graphs:
            ...
            bpy.context.scene.sublender_settings.active_graph = (
                importing_graph.graph_url
            )
            sbs_package = None
            ...
            if active_material_template != utils.consts.CUSTOM:
                ...
                for template_texture in material_template["texture"]:
                    if output_info_usage.get(template_texture) is not None:
                        ...
                        setattr(
                            graph_setting, utils.format.sb_output_to_prop(name), True
                        )
                ...
                for output_info in clss_info["output_info"]["list"]:
                    setattr(
                        graph_setting,
                        utils.format.sb_output_to_prop(output_info["name"]),
                        True,
                    )
