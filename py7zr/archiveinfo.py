...
from .compressor import SevenZipCompressor, SevenZipDecompressor
from .exceptions import Bad7zFile
from .helpers import ArchiveTimestamp, calculate_crc32
from .properties import ENCODED_HEADER_DEFAULT, ENCRYPTED_HEADER_DEFAULT, MAGIC_7Z, Property
...
class Folder:
    ...
    def write(self, file: BinaryIO):
        ...
        if sum([c['numinstreams'] for c in self.coders]) - sum(
                [c['numoutstreams'] for c in self.coders]) > 0:  # pragma: no-cover  # noqa
...
class UnpackInfo:
    ...
    def _retrieve_coders_info(self, file: BinaryIO):
        ...
        if pid == Property.CRC:
            ...
            pid = file.read(1)
        if pid != Property.END:
            raise Bad7zFile('end id expected but 0x{:02x} found at 0x{:08x}'.format(ord(pid),
                                                                                    file.tell()))  # pragma: no-cover  # noqa
