...
from .archiveinfo import Folder, Header, SignatureHeader
from .callbacks import ExtractCallback
from .compressor import get_methods_names_string
from .exceptions import Bad7zFile, CrcError, DecompressionError, InternalError, UnsupportedCompressionMethodError
from .helpers import ArchiveTimestamp, MemIO, NullIO, calculate_crc32, filetime_to_dt, readlink
from .properties import ARCHIVE_DEFAULT, ENCRYPTED_ARCHIVE_DEFAULT, MAGIC_7Z, READ_BLOCKSIZE
...
class SevenZipFile(contextlib.AbstractContextManager):
    ...
    def _real_get_contents(self, password) -> None:
        ...
        if hasattr(self.header, 'main_streams') and self.header.main_streams is not None:
            ...
            unpacksizes = subinfo.unpacksizes if subinfo.unpacksizes is not None else [x.unpacksizes[-1] for x in
                                                                                       folders]
        else:
            subinfo = None
            folders = None
    ...
    def _get_fileinfo_sizes(self, pstat, subinfo, packinfo, folder, packsizes, unpacksizes, file_in_solid,
                            numinstreams):
        if pstat.input == 0:
        ...
        if os.name == 'nt':
            ...
            if target.is_symlink():
                if dereference:
                    ...
                    f['emptystream'] = False
                    f[
                        'attributes'] = stat.FILE_ATTRIBUTE_ARCHIVE | stat.FILE_ATTRIBUTE_REPARSE_POINT  # type: ignore  # noqa
