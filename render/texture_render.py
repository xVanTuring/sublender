...
class SublenderOTRenderTexture(async_loop.AsyncModalOperatorMixin, bpy.types.Operator):
    ...
    async def import_graph(self, context):
        ...
        for importing_graph in importing_graph_items:
            ...
            param_list = generate_cmd_list(preferences, material_name, m_sublender, clss_info, graph_setting)
    ...
    async def update_texture(self, context):
        ...
        if self.texture_name == "":
            ...
            dep_name = utils.format.sb_output_dep_to_prop(self.texture_name)
            bit_depth = getattr(graph_setting, dep_name, "0")
            if bit_depth != "0":
                param_list.append("--output-bit-depth"),
                param_list.append(bit_depth)
            format_name = utils.format.sb_output_format_to_prop(self.texture_name)
            output_format = getattr(graph_setting, format_name, "png")
