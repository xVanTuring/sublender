...
def sbsar_input_updated_uid(input_id: str):
    def fn(_, context):
        if (
            context.scene.sublender_settings.live_update
...
def generate_sub_panel(group_map, graph_url):
    for group_key in group_map.keys():
        ...
        displace_name = group_name_map.get(
            cur_group["nameInShort"], cur_group["nameInShort"]
        )
        ...
        p_clss = type(
            format.sub_panel_name(group_key, graph_url),
    ...
    if globalvar.graph_clss.get(clss_name) is None:
        ...
        for input_info in all_inputs:
            ...
            if input_info["type"] == parser.sbsarlite.SBSARTypeEnum.INTEGER1:
                ...
                if (
                    input_info.get("widget") == "combobox"
                    and input_info.get("combo_items") is not None
                ):
        ...
        def parse_output(_output):
            _anno_obj[format.sb_output_to_prop(_output["identifier"])] = BoolProperty(
                name=_output["label"],
                default=False,
                update=sbsar_output_updated_name(_output["identifier"]),
            )
            _anno_obj[
                format.sb_output_format_to_prop(_output["identifier"])
            ] = EnumProperty(name="Format", items=consts.format_list, default="png")
            _anno_obj[
                format.sb_output_dep_to_prop(_output["identifier"])
            ] = EnumProperty(
                name="Bit Depth", items=consts.output_bit_depth, default="0"
            )

        output_list_dict, output_list, output_usage_dict = graph_output_parse(
            all_outputs, parse_output
        )
        ...
        clss = type(
            clss_name, (bpy.types.PropertyGroup,), {"__annotations__": _anno_obj}
        )
        register_class(clss)
        ...
        globalvar.graph_clss[clss_name] = {
            ...
            "group_info": {"tree": group_tree, "map": group_map},
            "output_info": {
                "list": output_list,
...
async def gen_clss_from_material_async(
    ...
    if sbs_package is not None:
        ...
        if enable_visible_if:
            ...
            ] = helper_class.EvalDelegate(target_material.name, clss_name)
...
def reset_material(material):
    ...
    for p_input in clss_info["input"]:
        if (
            p_input["identifier"] != "$outputsize"
            and p_input["identifier"] != "$randomseed"
        ):
